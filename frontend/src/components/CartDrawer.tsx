...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className="fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-[101] flex flex-col transform transition-transform duration-300 ease-in-out"
      >
        {/* Header */}
              ...
              {enrichedItems.map((item) => (
                <li key={item.variantId} className="flex gap-4 group">
                  <div className="relative w-24 h-24 bg-gray-50 rounded-xl overflow-hidden flex-shrink-0 border border-gray-100">
                    {item.cachedImage ? (
                      <Image src={item.cachedImage} alt={item.cachedTitle} fill className="object-contain p-2" />
                    ) : (
                    ...
                    <div>
                      <div className="flex justify-between items-start">
                        <Link 
                          ...
                          onClick={() => setIsCartOpen(false)}
                          className="text-sm font-semibold text-gray-900 hover:text-blue-600 line-clamp-2 pr-4 transition-colors"
                        >
                          {item.cachedTitle}
                        </Link>
                        <button 
                      ...
                      {item.cachedAttributes && (
                        <p className="text-xs text-gray-500 mt-1">{item.cachedAttributes}</p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center border border-gray-200 rounded-lg bg-white">
                        ...
                        <button 
                          ...
                          className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-gray-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          +
                        </button>
                      </div>
          ...
          <div className="border-t border-gray-100 p-6 bg-gray-50/50">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link 
                href="/cart"
                onClick={() => setIsCartOpen(false)}
                className="flex items-center justify-center py-3.5 px-4 rounded-xl border border-gray-200 bg-white text-gray-900 font-medium hover:bg-gray-50 transition-colors"
              >
                View Cart
              </Link>
              <Link 
                href="/checkout"
                onClick={() => setIsCartOpen(false)}
                className="flex items-center justify-center py-3.5 px-4 rounded-xl bg-gray-900 text-white font-medium hover:bg-gray-800 transition-colors"
              >
                Checkout
              </Link>
            </div>
